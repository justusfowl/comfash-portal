...
@Component({
    ...
    template: `
<div #votePopover class="popover {{ effectivePlacement }}"
     [style.top]="top + 'px'"
     [style.left]="left + 'px'"
     [class.in]="isIn"
...
export class PopoverContent implements AfterViewInit, OnDestroy {
    ...
    show(): void {
        ...
        const p = this.positionElements(this.popover.getElement(), this.placement);
    ...
    // Protected Methods
    // -------------------------------------------------------------------------
    protected positionElements(hostEl: HTMLElement, positionStr: string, appendToBody: boolean = false): { top: number, left: number } {
        let positionStrParts = positionStr.split("-");
        let pos0 = positionStrParts[0];
        let pos1 = positionStrParts[1] || "center";
        let hostElPos = appendToBody ? this.offset(hostEl) : this.position(hostEl);
        let targetElWidth = this.targetWidth;
        let targetElHeight = this.targetHeight;

        this.effectivePlacement = pos0 = this.getEffectivePlacement(pos0, hostEl);
        ...
        let originalElement = hostEl.getBoundingClientRect(); 



        targetElPos = {
            top: - (this.targetHeight) * 1.05,
            left: - (this.targetWidth / 2) + (hostElPos.left + (hostElPos.width / 2))
        };
    ...
    protected getEffectivePlacement(placement: string, hostElement: HTMLElement): string {
        ...
        if (desiredPlacement === "top" && hostElBoundingRect.top - this.targetHeight < 0) {
            return "bottom";
        }
        if (desiredPlacement === "bottom" && hostElBoundingRect.bottom + this.targetHeight > window.innerHeight) {
            return "top";
        }
        if (desiredPlacement === "left" && hostElBoundingRect.left - this.targetHeight < 0) {
            return "right";
        }
        if (desiredPlacement === "right" && hostElBoundingRect.right + this.targetHeight > window.innerWidth) {
            return "left";
        }
